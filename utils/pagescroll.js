class PageScroll {
  ...
  #renderNav() {
    let arrows = `<svg id='control-up' width="40" height="35" viewBox="0 0 40 35" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M17.4019 1.5C18.5566 -0.5 21.4434 -0.5 22.5981 1.5L39.0526 30C40.2073 32 38.7639 34.5 36.4545 34.5H3.54552C1.23612 34.5 -0.207259 32 0.947441 30L17.4019 1.5Z" fill="${
              this.opts.controlColor || "white"
            }"/>
        </svg>
    ...
    this.$sections.forEach(
      (elem) =>
        (elem.style.transition = `transform ${
          +this.opts.animDuration / 1000 + "s" || "0.3s"
        }`)
    )
    this.$sections.forEach(
      (elem) =>
        (elem.style.transitionTimingFunction = this.opts.easing
          ? this.opts.easing
          : "linear")
    )
  ...
  moveup() {
    if (this.level != 0 && !this.isAnimating) {
      ...
      this.level--
      this.$sections.forEach(
        (elem) => (elem.style.transform = `translateY(${-this.level * 100}vh)`)
      )
      setTimeout(() => {
        this.isAnimating = false
